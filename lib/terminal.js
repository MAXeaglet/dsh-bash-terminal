...
export function terminalTool(ctx, registry, paths, resolveShell) {
  return defineTool({
    name: "terminal",
    description: "Interactive terminal session over the user's terminal choice for this tool (Settings -> General -> Terminal 工具默认终端: powershell / gitbash / wsl); when it is unset the session follows the shell tool's default terminal. A real PTY hosts a persistent shell: open a session, send input and read output across turns, deliver signals (Ctrl+C = SIGINT) to the foreground process, and close when done. The session survives between calls until closed and is managed as a background job (job_kill / job_output work on it); idle sessions close automatically. Use this for interactive programs (REPLs, ssh, databases, TUI tools) or when you need shell state (cwd, variables, aliases) to persist across calls.",
    parameters: {
      action: {
        type: "string",
      ...
      distro: {
        type: "string",
        description: "WSL distribution (only when the session's terminal shell is wsl)."
      },
      input: {
        type: "string",
