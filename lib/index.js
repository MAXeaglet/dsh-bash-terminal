...
function candidateExists(candidate) {
  try {
    // Follow links and require a real, non-empty file. Windows app-execution
    // aliases (e.g. %LOCALAPPDATA%\Microsoft\WindowsApps\bash.exe, the WSL
    // forwarder) are 0-byte reparse points: lstat() reports them as symlinks,
    // but resolving them fails with EACCES and they are never a Git Bash.
    // Following the link keeps genuine symlinked installs (scoop/chocolatey
    // shims) working.
...
/**
 * True when `entry` is `dir` itself or a descendant of it. Compares path
 * components instead of substrings, so a sibling whose name merely starts with
 * the same text (e.g. `...\\Microsoft\\WindowsAppsBackup`) is not excluded.
 */
function isUnder(entry, dir) {
  const e = entry.toLowerCase().replace(/[\\/]+$/, "");
  const d = dir.toLowerCase().replace(/[\\/]+$/, "");
  return e === d || e.startsWith(d + "\\") || e.startsWith(d + "/");
}
 ...
 * System32 launcher (c:\\windows\\system32\\bash.exe) and the WindowsApps
 * alias store (%LOCALAPPDATA%\\Microsoft\\WindowsApps) — both are WSL
 * forwarders, neither is a Git Bash shell.
 */
export function candidateGitBashPaths(env = process.env) {
  ...
  // 0-byte app-execution aliases live here; their bash.exe forwards to WSL.
  const windowsApps = localAppData.length > 0 ? join(localAppData, "Microsoft", "WindowsApps").toLowerCase() : "";
  for (const entry of (env.PATH ?? "").split(";")) {
    const trimmed = entry.trim().replace(/^"|"$/g, "");
    if (trimmed.length === 0) continue;
    if (isUnder(trimmed, systemRoot)) continue;
    if (windowsApps.length > 0 && isUnder(trimmed, windowsApps)) continue;
