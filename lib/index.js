...
export const Config = z.object({
  ...
  /** 交互式 terminal 工具专用默认终端；""（默认）= 跟随 defaultShell（本地分设改动）。 */
...
export function apply(ctx, config = {}) {
  ...
  const terminalShell = config.terminalShell ?? "";
  // "" = 跟随 defaultShell；其余值必须是 SHELLS 成员（apply 期即拒绝，而不是等运行时拿非法值拼 argv）。
  if (terminalShell !== "" && !SHELLS.includes(terminalShell)) {
    throw new Error(`dsh-bash-terminal: invalid terminalShell ${JSON.stringify(terminalShell)} (expected "" or one of ${SHELLS.join(", ")})`);
  }
  const settingsScope = ctx.settings.register(
    SETTINGS_NAMESPACE,
    z.object({
      defaultShell: z.union(SHELLS.map((s) => z.const(s))).default(defaultShell),
      // terminalShell 的 UI 语义："" = 跟随 defaultShell（未单独配置时行为与上游一致）。
      // union 限定到合法值，让设置写入端对外部/界面写入直接校验。
      // 默认值与 base 都取 config.terminalShell（与 defaultShell 同构）——只写 default("")、
      // 且 base 里漏掉它的话，插件 config 里的 terminalShell 会被静默忽略。
      terminalShell: z.union([z.const(""), ...SHELLS.map((s) => z.const(s))]).default(terminalShell)
    }),
    { base: { defaultShell, terminalShell } }
  );
  ...
  // terminal 工具的默认终端：terminalShell 非空用它；否则跟随 shell 工具的 defaultShell
  //（每个调用动态读 settings，settings.yaml 热更后即时生效）。
  const terminalShellOf = () => {
    const section = settingsScope.get();
    return section.terminalShell !== undefined && section.terminalShell.length > 0
      ? section.terminalShell
      : section.defaultShell;
  };
  ctx.tools.register(terminalTool(ctx, terminalRegistry, paths, terminalShellOf));
