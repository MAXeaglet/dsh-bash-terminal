...
export const inject = ["tools", "systemPrompt", "shellEnv", "subprocess", "settings"];
...
function toolDescription(backgroundEnabled) {
  const base = [
    "Execute a command in a Windows terminal. The terminal backend (powershell / gitbash / wsl) is chosen by the user in the Web UI settings (default terminal) — never guess or override it; use whatever the user configured. Backends:",
...
export function apply(ctx, config = {}) {
  ...
  ctx.systemPrompt.section({
    ...
    order: 105,
    text: "Check the [exit code: N] marker on every shell result; investigate failures before moving on."
  });
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const argv = buildArgv(shell, args.command, paths, args.distro);
      if (argv[0] === undefined) {
