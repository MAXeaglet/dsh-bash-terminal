...
- **User decides, the AI cannot override**: Settings -> General exposes **two** rows — **Shell tool default terminal** (`defaultShell`) and **Terminal tool default terminal** (`terminalShell`, whose options include "Follow the shell tool" = empty). The `shell` tool obeys the former; the interactive `terminal` tool obeys the latter, falling back to the former when unset. Neither exposes a per-call terminal argument to the model, and both persist through the DSH settings system.
- **Why split them**: batch execution and interactive sessions often want different backends — on Windows a restricted sandbox makes MSYS2/Git Bash abort with `CreateFileMapping` Win32 error 5, so `shell` prefers PowerShell while `terminal` still opens a real Git Bash.
